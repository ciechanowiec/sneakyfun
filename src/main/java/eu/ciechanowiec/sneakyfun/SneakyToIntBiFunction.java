...
public interface SneakyToIntBiFunction<T, U, X extends Exception> {
     ...
     */
    int applyAsInt(T inputOne, U inputTwo) throws X;
     ...
     *   </li>
     * </ol>
     * @param toIntBiFunction sneaky functional interface that will be wrapped into the analogous
    ...
    static<T, U, X extends Exception> ToIntBiFunction<T, U> sneaky(SneakyToIntBiFunction<T, U, X> toIntBiFunction) {
        Objects.requireNonNull(toIntBiFunction);
        return (inputOne, inputTwo) -> {
            try {
                return toIntBiFunction.applyAsInt(inputOne, inputTwo);
