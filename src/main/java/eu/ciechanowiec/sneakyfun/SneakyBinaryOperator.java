 ...
 * {@link SneakyBiFunction} for the case where the operands and the result are all of
 * the same type.
 *
...
public interface SneakyBinaryOperator<T, X extends Exception> extends SneakyBiFunction<T, T, T, X> {
     ...
     *   </li>
     * </ol>
     * @param binaryOperator sneaky functional interface that will be wrapped into the analogous
    ...
    static<T, X extends Exception> BinaryOperator<T> sneaky(SneakyBinaryOperator<T, X> binaryOperator) {
        ...
        return (inputOne, inputTwo) -> {
            try {
                return binaryOperator.apply(inputOne, inputTwo);
